  ...
  void symtotok(SYMBOL symbolVal, TOKEN tok){
    ...
    if (symbolVal->kind == CONSTSYM && symbolVal->basicdt == INTEGER ) {
      ...
      if (symbolVal->basicdt == REAL){
        tok -> intval = symbolVal->constval.realnum;
      } else if (symbolVal->basicdt == INTEGER){
        tok -> realval = symbolVal->constval.intnum;
      }
    }
  }
  ...
  TOKEN binop(TOKEN op, TOKEN lhs, TOKEN rhs) {     
    ...
    if (lhs->stringval != NULL){
      lsym = searchst(lhs->stringval);
    }
    if (rhs->stringval != NULL){
      rsym = searchst(rhs->stringval);
    }
    if (lsym) {
      lhs->symentry = lsym;
      getSym(lsym, lhs);
    ...
    if (op->whichval == ASSIGNOP && lhsInt && rhsFloat) {
      TOKEN sym;
      ...
      rhs = sym;
    } else if (!op->whichval == ASSIGNOP &&  lhsInt && rhsFloat) {
      lhs  = makefloat(lhs);
    }
    cons(rhs, NULL);
    cons(lhs, rhs);
